...
Unpacker_Thread_Pool::Unpacker_Thread_Pool(){
    Start_Filling_UP();
};

void Unpacker_Thread_Pool::Infinite_loop_function() {
...
void Unpacker_Thread_Pool::Add_Job(std::shared_ptr<Job> new_job) {
    {
...
void Unpacker_Thread_Pool::Add_Jobs(std::vector<std::shared_ptr<Job>> new_jobs){
    {
        unique_lock <mutex> lock(queue_mutex);
...
void Unpacker_Thread_Pool::Stop_Filling_UP(){
    filling_up = false;
}

void Unpacker_Thread_Pool::Start_Filling_UP(){
    filling_up = true;
}

std::shared_ptr<Job> Unpacker_Thread_Pool::Request_Job(){
    unique_lock <mutex> lock(queue_mutex);
    while(true){
        bool all_done = true;
        for (auto & j: queue){
