...
DBManager::DBManager() {
    ...
    DBManager::con = shared_ptr<Connection>(driver->connect(host, user, password));
...
void DBManager::insert_node(const DBStruct::node &n){
  try{
...
DBStruct::node DBManager::get_node(const std::string k){
  DBStruct::node n;
...
std::vector<std::string> DBManager::get_all_hash(){
...
void DBManager::delete_node(const std::string k){
  try{
